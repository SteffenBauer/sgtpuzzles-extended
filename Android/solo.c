...
static int game_fetch_preset(int i, char **name, game_params **params)
{
    static struct {
        ...
        { "3x3 Advanced X",    { 3, 3, SYMM_NONE, DIFF_SET, DIFF_KMINMAX, TRUE } },
        { "3x3 Killer",        { 3, 3, SYMM_NONE, DIFF_BLOCK, DIFF_KINTERSECT, FALSE, TRUE } },
        { "5 Jigsaw Advanced", { 5, 1, SYMM_NONE, DIFF_SET, DIFF_KMINMAX, FALSE, FALSE } },
...
static game_ui *new_ui(const game_state *state)
{
    ...
    ui->hx = ui->hy = 0;
    ui->hpencil = ui->hshow = ui->hcursor = ui->hhint = 0;
    return ui;
}
                            ...
                            int x, int y, int button)
{
    ...
    if (tx >= 0 && tx < cr && ty >= 0 && ty < cr) {
        if (button == LEFT_BUTTON) {
            ...
            else if (ui->hhint != 0 && state->pencil[(ty*cr+tx) * cr + (ui->hhint-1)]) {
        ...
        if (button == RIGHT_BUTTON) {
             ...
             */
            if (ui->hhint != 0 && state->pencil[(ty*cr+tx) * cr + (ui->hhint-1)]) {
...
static int check_hint(const game_state *state, int x, int y, int n) {
    int cr = state->cr;
    int cx,cy,i;
    int check;
    ...
    if (state->kblocks) {
        if (!check) 
            for (i=0;i<state->kblocks->nr_squares[state->kblocks->whichblock[y*cr+x]];i++)
                if (state->grid[state->kblocks->blocks[state->kblocks->whichblock[y*cr+x]][i]] == n) {check = TRUE; break; }
    }
        ...
        for (y=0;y<cr;y++) {
            if (ret->grid[y*cr+x] == 0) 
                for (n=1;n<=cr;n++) {
                    ...
                    else if (move[0] == '-' && ret->pencil[(y*cr+x) * cr + (n-1)] && check_hint(from,x,y,n)) 
