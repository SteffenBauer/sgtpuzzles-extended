...
static char *validate_params(const game_params *params, int full)
{
    ...
    if (params->w < l || params->h < l) {
        sprintf(err, "Width and height for this grid type must both be at least %d", l);
        return err;
    }
    l = grid_size_limits[params->type].omin;
    if (params->w < l && params->h < l) {
        sprintf(err, "At least one of width and height for this grid type must be at least %d", l);
        return err;
    }
