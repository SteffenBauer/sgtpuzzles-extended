 ...
 *  certain grid position. The internal generator does not use
...
static int game_fetch_preset(int i, char **name, game_params **params) {
    ...
    *params = ret;
    ...
    sprintf(buf, undead_presets[i].stripclues
            ? "%dx%d %s, strip clues" : "%dx%d %s",
...
#ifdef ANDROID
    android_keys("GVZ\bA", ANDROID_ARROWS);
#endif
...
#ifdef ANDROID
    if (newstate->solved && oldstate && !oldstate->solved && !oldstate->cheated && !newstate->cheated) android_completed();
#endif
}
...
static void draw_path_hint(drawing *dr, game_drawstate *ds,
    ...
    sightings = start ? state->common->paths[i].sightings_start : 
                        state->common->paths[i].sightings_end;
    if (sightings >= 0) sprintf(buf,"%d", sightings);
    else                sprintf(buf," ");
    
    draw_rect(dr,dx-(TILESIZE/2)+2,dy-(TILESIZE/2)+2,TILESIZE-3,TILESIZE-3,
              COL_BACKGROUND);
    draw_text(dr,dx,dy,FONT_FIXED,TILESIZE/2,ALIGN_HCENTRE|ALIGN_VCENTRE, 
              error ? COL_ERROR : hflash ? COL_FLASH : COL_TEXT,buf);
