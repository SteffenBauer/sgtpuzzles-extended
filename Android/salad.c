...
static const struct game_params salad_presets[] = {
	{4, 3, GAMEMODE_LETTERS, DIFF_HARD},
...
static game_state *new_game(midend *me, const game_params *params, const char *desc)
{
    ...
    keys[params->nums   ] = 'X';
    keys[params->nums +1] = 'O';
    keys[params->nums +2] = '\b';
    keys[params->nums +3] = 'M';
    keys[params->nums +4] = '\0';
...
static char *salad_new_numbers_desc(const game_params *params, random_state *rs, char **aux)
{
	...
	ret = salad_serialize(state->gridclues, o2, '0');
	free_game(state);
...
static char *salad_new_letters_desc(const game_params *params, random_state *rs, char **aux)
{
	...
	/* Encode game */
	borderstr = salad_serialize(state->borderclues, ox4, 'A' - 1);
	
	gridstr = salad_serialize(state->gridclues, o2, 'A' - 1);
	free_game(state);
	ret = snewn(ox4+o2+2, char);
...
{
#ifdef ANDROID
    if (newstate->completed && oldstate && !oldstate->completed && !oldstate->cheated && !newstate->cheated) android_completed();
#endif
}
...
static game_state *execute_move(const game_state *state, const char *move)
{
	...
	if (move[0] == 'M') {
		...
		for (y=0;y<o;y++)
		for (x=0;x<o;x++) {
			if (ret->grid[y*o+x] == 0 && ret->holes[y*o+x] == 0) {
				int i;
				ret->marks[y*o+x] = 0x00;
				for (i=0;i<nums;i++) {
					ret->marks[y*o+x] <<= 1;
					ret->marks[y*o+x] |= 0x01;
				}
			}
		}
		return ret;
...
int main(int argc, char *argv[])
{
	...
	if (!desc)
	{
		...
		for(i = 0; i < attempts; i++)
		{
			...
			state = load_game(params, desc_gen, &fail);
			if(fail)
			{
