                            ...
                            const char *desc)
{
    ...
    if (state->manual && strcmp(desc, "zzzc") == 0) {
        current_midend = me;
        state->fixed = false;
    }
    else {
                            ...
                            int x, int y, int button)
{
    ...
    if (tx >= 0 && tx < cr && ty >= 0 && ty < cr) {
        if (button == LEFT_BUTTON) {
            if (state->immutable[ty*cr+tx] && !fixed_entry) {
                ui->hshow = false;
            } else if (ui->hhint != 0 && !fixed_entry &&
    ...
    if (ui->hshow &&
        ...
        /*
         * Can't overwrite this square in normal mode. This can only happen here
         * if we're using the cursor keys, or if we are in manual entry mode.
         */
    ...
    if (move[0] == '+' || move[0] == '-') {
        ...
        if (move[0] == 'P' && n > 0) {
            ...
            if (ret->manual && !ret->fixed)
                ret->immutable[y*cr+x] = (move[0] == 'F') && (n>0);
...
static float *game_colours(frontend *fe, int *ncolours)
{
    ...
    ret[COL_FILLHIGHLIGHT * 3 + 0] = 0.9F * ret[COL_BACKGROUND * 3 + 0];
    ret[COL_FILLHIGHLIGHT * 3 + 1] = 0.9F * ret[COL_BACKGROUND * 3 + 1];
    ret[COL_FILLHIGHLIGHT * 3 + 2] = 0.9F * ret[COL_BACKGROUND * 3 + 2];
